...
const CourseList = () => {
    const [acadamicYearTableList,setAcadamicYearTableList]=useState([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <a onClick={()=>router.push(`/acadamic_year/${record.id}/info`)}>{record.acadamic_year}</a> 
        </Space>
      ),
...

    {
        ...
        render: (_, record) => (
          <Space size="middle"> 
            <a onClick={()=>router.push(`/acadamicyear/${record.id}/info`)}>Info</a> 
            <a onClick={()=>router.push(`/acadamicyear/${record.id}/update`)}>Update</a> 
            <a onClick={()=>deleteList(`/acadamic_year/${record.id}`)}>Delete</a> 
          </Space>
        ),
      },
  ];
 ...
 const  deleteList = async (courseId) => {
  ...
  const onFinish = async () => {
    try {
      ...
      setAcadamicYearTableList(result); 
      console.log(acadamicYearTableList)
    } catch (error) {
  ...
  return (
    <>
      <Layouts>
         ...
         <Title  className="mb-15">Acadamic Year</Title>
